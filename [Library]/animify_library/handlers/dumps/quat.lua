...
function ApplyElementRotation(element, rotX, rotY, rotZ, relativeToWorld)
  ...
  --setElementRotation(element,rot.x,rot.y,rot.z,)
...
local pedElement = false --TODO: CHANGE THIS TO LOCALPLAYER OR WHATEVER

function ApplyElementBoneRotation(boneID, rotX, rotY, rotZ, relativeToWorld)

    pedElement = cinemationData.pedData.createdPed
    element = pedElement
    if rotX == 0.0 and rotY == 0.0 and rotZ == 0.0 then
      return
    end
    ...
    local elementPosition = element.matrix
    local vecX = Vector3(1, 0, 0)
    local vecY = Vector3(0, 1, 0)
    local vecZ = Vector3(0, 0, 1)
    ...
    local rot = QuaternionToEuler(inQuat[1], inQuat[2], inQuat[3], inQuat[4])
    boneTest[boneID] = rot
    --setElementBoneRotation(element,boneID,rot.x,rot.y,rot.z)
  ...
  addEventHandler("onClientPedsProcessed", root, function()
    ...
    if pedElement then
        for i, j in pairs(boneTest) do
            setElementBoneRotation(pedElement, i, j.z, j.x, j.y)
        end
        updateElementRpHAnim(pedElement)
    end
